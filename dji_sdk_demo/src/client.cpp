...
double thresh_pixel;
double thresh_pattern_y;
double thresh_pattern_z;
double thresh_gap;
double k_p;
double k_d;
double k_i;
   ...
   for( size_t i = 0; i < contours.size(); i++ )
    {	
        ...
        if(contours_poly[i].size() == 4 && fabs(contourArea(Mat(contours_poly[i]))) > 1000 ) //&& isContourConvex(Mat(contours_poly[i]))
        {
            ...
            if( maxCosine < 0.3 && hierarchy[i][2] == -1 && hierarchy[i][3] != -1 )
               {
               //with father contour but no child 
                    ...
                    ret_val.z = fabs(contours_poly[i][0].x - contours_poly[i][2].x);
                }    
            else
               {
                    ret_val.x = 99999;
...
{ 
    ...
    nh1.param("thresh_pattern_y",thresh_pattern_y,10.0);
    nh1.param("thresh_pattern_z",thresh_pattern_z,3.0);
    nh1.param("k_p",k_p,0.5);
    nh1.param("k_d",k_d,0.05);
    nh1.param("k_i",k_i,0.005);
    nh1.param("v_max_y",v_max_y,0.3);
    nh1.param("v_max_z",v_max_z,0.15);
    ...
    nh1.param("pattern_width",pattern_width,60.0);//unit: cm
    ...
    while(ros::ok())
    {
      ...
      if (contoursCenter.z !=0)
        {
          detected_flag = 1;
          pattern_width_in_pixel = contoursCenter.z;  
        }
      ...
      if(through_flag == 1)
      {
        ...
        {//``````step 2: target detected, adjust drone to aim at contours center
          ...
          if (contoursCenter.x == 99999 ||(contoursCenter.x == 0 && contoursCenter.y == 0) )
             ...
             {
                 e_Y = (contoursCenter.x - p_x) * pattern_width / pattern_width_in_pixel;
                 e_Z = -1 * (contoursCenter.y - p_y) * pattern_width / pattern_width_in_pixel;//coordinate frame transformation
             }
